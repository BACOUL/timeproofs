...
const communityPublicationCodexPreflightSteps = [
  ...
  "prepare npm login --auth-type=web for JEASON's temporary local owner login, but do not execute it",
  "prepare the exact npm publish command",
  "stop before any owner 2FA entry"
];
const communityPublicationOwnerCheckpointSteps = [
  "JEASON runs npm login --auth-type=web in his own local Termux terminal if not already authenticated",
  "JEASON allows npm CLI to store the temporary owner login only in his personal ~/.npmrc on that device",
...
decisionGate("AR-COM-EPIC", "AR-COM-006B", "M3", "BEFORE_COMMUNITY_PUBLICATION", "COM", "Decide handling of unexpected npm latest dist-tag", { status: "DECISION_REQUIRED", owner: "JEASON", depends_on: ["AR-COM-001", "AR-COM-002", "AR-COM-003", "AR-COM-004", "AR-COM-005", "AR-COM-006A"], blocks: ["AR-COM-006"], source_documents: [doc.policy, doc.release], acceptance_criteria: ["owner decision recorded for unexpected latest dist-tag", "decision determines whether tag and GitHub Release may proceed", "no further npm, Git tag or GitHub Release action occurs before decision"], required_evidence: ["owner decision on latest deviation", "accepted remediation or continuation plan"], metrics: ["npm publication success", "alpha dist-tag observed", "unexpected latest dist-tag observed", "latest removal failed with E400"], notes: "npm publication succeeded under alpha, but npm also exposed latest: 0.1.0-alpha.0 unexpectedly. Removing latest failed with E400. Continuation is blocked before immutable Git tag or GitHub Release." });
codex("AR-COM-EPIC", "AR-COM-006", "M3", "BEFORE_COMMUNITY_PUBLICATION", "COM", "Publish Community CLI and immutable release", { status: "BLOCKED", owner: "CODEX_AND_JEASON", weight: 5, depends_on: ["AR-COM-001", "AR-COM-002", "AR-COM-003", "AR-COM-004", "AR-COM-005", "AR-COM-006A", "AR-COM-006B"], blocks: ["AR-COM-008", "AR-COM-007"], source_documents: [doc.policy, doc.release], branch: "release-agentready-community-cli", pr_title: "release(agentready): publish Community CLI and immutable release", allowed_paths: ["CHANGELOG.md", "docs/agentready/**"], forbidden_paths: ["package.json", "packaging/agentready-community/**", "agentready-core/**", "bin/**", ".github/workflows/**", ".github/actions/**", "LICENSE", "NOTICE", "*.html"], acceptance_criteria: ["Codex verifies the exact approved artifact before publication", "JEASON performs the manual npm publish checkpoint with private owner 2FA", "publication occurs only under npm dist-tag alpha", "latest is not created, moved or modified", "immutable tag and GitHub Release are created only after npm publication is confirmed", "public install tested"], required_commands: ["node cli/tests/run-agentready-package-smoke-test.mjs"], required_evidence: ["approved tarball SHA-256 verification", "JEASON npm publication confirmation without secrets", "npm package URL", "immutable tag", "GitHub Release URL", "public installation test"], evidence: [{ type: "npm_publication_result", date: "2026-07-12", package: "@timeproofs/agentready", version: "0.1.0-alpha.0", command: "npm publish ./timeproofs-agentready-0.1.0-alpha.0-approved.tgz --access public --tag alpha", result: "SUCCESS", confirmation: "+ @timeproofs/agentready@0.1.0-alpha.0", tarball_sha256: "602799c5dd20ada03f2ee5e27048bacd865a71654e1c09f8119a484c837da6fe", alpha_dist_tag: "0.1.0-alpha.0", latest_dist_tag: "0.1.0-alpha.0", latest_expected: false, latest_removal_attempted: true, latest_removal_command: "npm dist-tag rm @timeproofs/agentready latest", latest_removal_result: "E400 400 Bad Request DELETE /-/package/@timeproofs%2fagentready/dist-tags/latest", logout_executed: true, authentication_status: "LOGGED_OUT", continuation_authorized: false, git_tag_created: false, github_release_created: false, notes: "npm publication succeeded, but latest was unexpectedly created and could not be removed. Owner decision required before any Git tag or GitHub Release." }], manual_actions: communityPublicationManualActions, authorized_actions: communityPublicationAuthorizedActions, forbidden_actions: communityPublicationForbiddenActions, codex_preflight_steps: communityPublicationCodexPreflightSteps, owner_checkpoint_steps: communityPublicationOwnerCheckpointSteps, post_confirmation_steps: communityPublicationPostConfirmationSteps, scope_justification: "Weight 5 justified: release publication is a single atomic release boundary after owner approval and private owner 2FA checkpoint." });
for (const row of [
...
const batchDefinitions = [
  ["ARB-COM-001", "Publish Community CLI and immutable release", ["AR-COM-006"], { status: "BLOCKED", owner: "CODEX_AND_JEASON", spec_status: "EXECUTION_READY", pr_number: 124, evidence: [{ type: "partial_publication", pr: 124, npm_publication: "SUCCESS", package: "@timeproofs/agentready", version: "0.1.0-alpha.0", alpha_dist_tag: "0.1.0-alpha.0", unexpected_latest_dist_tag: "0.1.0-alpha.0", latest_removal_result: "E400", git_tag_created: false, github_release_created: false, continuation_authorized: false }], notes: "Blocked after npm publication because latest was unexpectedly set to 0.1.0-alpha.0 and latest removal failed with E400. Owner decision AR-COM-006B is required before tag or GitHub Release.", scope_justification: "Atomic release boundary after owner, legal and private 2FA checkpoint approval." }],
