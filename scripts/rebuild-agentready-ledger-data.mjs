...
ownerAction("AR-COM-EPIC", "AR-COM-001", "M2", "BEFORE_COMMUNITY_PUBLICATION", "OWNER", "Verify control of npm scope @timeproofs", { depends_on: ["AR-GOV-003"], blocks: ["AR-COM-006"], source_documents: [doc.npm], acceptance_criteria: ["scope control verified with dated evidence", "package publish rights verified"], required_evidence: ["owner-provided npm scope evidence"], external_verification: { required: true, topic: "npm scope and package control", verified_at: null, valid_until: null, sources: [], verified_by: "", result: "" } });
ownerAction("AR-COM-EPIC", "AR-COM-002", "M2", "BEFORE_COMMUNITY_PUBLICATION", "OWNER", "Define npm account security", { depends_on: ["AR-GOV-003"], blocks: ["AR-COM-006"], source_documents: [doc.policy], acceptance_criteria: ["2FA or trusted publishing recorded", "no long-lived npm token stored"], required_evidence: ["owner-provided npm security evidence"], external_verification: { required: true, topic: "npm 2FA or trusted publishing", verified_at: null, valid_until: null, sources: [], verified_by: "", result: "" } });
legalReview("AR-COM-EPIC", "AR-COM-003", "M2", "BEFORE_COMMUNITY_PUBLICATION", "Approve AgentReady Community license", { depends_on: ["AR-GOV-003"], blocks: ["AR-COM-006"], source_documents: [doc.license, "LICENSE"], acceptance_criteria: ["final license approved", "package license and LICENSE coherent"], required_evidence: ["legal approval or owner legal decision"] });
legalReview("AR-COM-EPIC", "AR-COM-004", "M2", "BEFORE_COMMUNITY_PUBLICATION", "Treat package-public ProofSpec references", { depends_on: ["AR-GOV-003"], blocks: ["AR-COM-006"], source_documents: [doc.proofspec, "LICENSE", "README.md"], acceptance_criteria: ["legacy references removed, renamed or legally justified"], required_evidence: ["legal treatment decision", "updated package-public audit"] });
ownerAction("AR-COM-EPIC", "AR-COM-005", "M2", "BEFORE_COMMUNITY_PUBLICATION", "OWNER", "Approve final Community tarball content", { depends_on: ["AR-COM-003", "AR-COM-004"], blocks: ["AR-COM-006"], source_documents: [doc.tarball], acceptance_criteria: ["tarball file list reviewed", "checksum approved"], required_evidence: ["approved tarball SHA-256", "approved source commit"] });
decisionGate("AR-COM-EPIC", "AR-COM-006A", "M2", "BEFORE_COMMUNITY_PUBLICATION", "OWNER", "Explicit Community publication approval", { depends_on: ["AR-COM-001", "AR-COM-002", "AR-COM-003", "AR-COM-004", "AR-COM-005"], blocks: ["AR-COM-006"], source_documents: ["docs/agentready/COMMUNITY_PUBLICATION_APPROVAL_CHECKLIST.md"], acceptance_criteria: ["PUBLICATION APPROVED explicitly set by owner"], required_evidence: ["approved commit", "approved version", "approved tarball SHA-256", "approval date"], metrics: ["all blockers RESOLVED", "tarball approved", "license approved"] });
codex("AR-COM-EPIC", "AR-COM-006", "M3", "BEFORE_COMMUNITY_PUBLICATION", "COM", "Publish Community CLI and immutable release", { status: "BLOCKED", weight: 5, depends_on: ["AR-COM-001", "AR-COM-002", "AR-COM-003", "AR-COM-004", "AR-COM-005", "AR-COM-006A"], blocks: ["AR-COM-008", "AR-COM-007"], source_documents: [doc.policy, doc.release], branch: "release-agentready-community-cli", pr_title: "release(agentready): publish Community CLI and immutable release", allowed_paths: ["package.json", "CHANGELOG.md", "docs/agentready/**"], forbidden_paths: ["agentready-core/**", "bin/**", "*.html", "LICENSE"], acceptance_criteria: ["publication only from approved commit", "immutable tag and GitHub Release only when approved", "public install tested"], required_commands: ["node cli/tests/run-agentready-package-smoke-test.mjs"], required_evidence: ["npm package URL", "immutable tag", "GitHub Release URL", "public installation test"], scope_justification: "Weight 5 justified: release publication is a single atomic release boundary after owner approval." });
for (const row of [
...
external("AR-SEO-EPIC","AR-SEO-004","M7","BEFORE_GLOBAL_LAUNCH","SEO","Verify search topics and intent externally","search topics and intent",{source_documents:[doc.seo]});
...
external("AR-COMP-EPIC","AR-COMP-001","M7","BEFORE_GLOBAL_LAUNCH","COMP","Verify competitors and pricing categories","competitors pricing and categories",{source_documents:[doc.comp]});
...
function walk(dir) { const out = []; if (!existsSync(dir)) return out; for (const entry of readdirSync(dir, { withFileTypes: true })) { const file = path.join(dir, entry.name).replaceAll(path.sep, '/'); if (entry.isDirectory()) out.push(...walk(file)); else if (entry.isFile() && entry.name.endsWith('.md')) out.push(file); } return out; }
