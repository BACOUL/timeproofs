...
for (const [id, title, branch, prTitle] of [
  ...
  weight: 5,
  source_documents: [doc.globalSite, doc.ia, doc.copy],
  branch,
  pr_title: prTitle,
  allowed_paths: ["assets/**", "*.html", "docs/agentready/**", "scripts/**", "sitemap.xml"],
  forbidden_paths: ["agentready-core/**", "bin/**", "package.json", "action.yml", ".github/workflows/**", "server/**", "api/**", "LICENSE", "NOTICE"],
  deliverables: [title],
  acceptance_criteria: [`${title} complete after preceding stacked site batch is reviewed`],
  rollback_boundary: `Revert ${id} without reverting earlier stacked site batches.`,
  scope_justification: "Weight 5 justified: planned global-site batch retained as a non-executable skeleton until the preceding stacked batch is reviewed."
});
...
const batchDefinitions = [
  ...
  ["ARB-SITE-PREMIUM-001", "Build global site shell navigation and footer", ["AR-SITE-PREMIUM-001"], { status: "READY", spec_status: "EXECUTION_READY", owner: "CODEX_AND_JEASON", pr_number: 132, base_branch: "timeproofs", depends_on_batches: ["ARB-COM-002"], evidence: [{ type: "stack_base", pr: 132, head_sha: "8ea9b08e9c772f151c3966288f7e82f8efe0ff10", approved_for_stacked_execution: true, merge_authorized: false }], notes: "The detailed work item remains IN_REVIEW in PR #132. This batch-level READY status preserves the existing Community Release Candidate guard until the stacked-site governance workflow is updated by a later approved change." }],
  ["ARB-SITE-GLOBAL-002", "Publish global product scanners CI and planned pricing foundation", ["AR-SITE-GLOBAL-002"], { status: "BLOCKED", spec_status: "EXECUTION_READY", owner: "CODEX_AND_JEASON", pr_number: 134, base_branch: "site-agentready-premium-foundation", stacked_execution_authorized: true, stacked_on_batch: "ARB-SITE-PREMIUM-001", stacked_base_pr: 132, stacked_base_head_sha: "8ea9b08e9c772f151c3966288f7e82f8efe0ff10", depends_on_batches: ["ARB-SITE-PREMIUM-001"], notes: "Draft PR #134 is open on the stacked child branch, but this batch remains blocked in the canonical sequence until ARB-SITE-PREMIUM-001 is merged and reconciled." }],
