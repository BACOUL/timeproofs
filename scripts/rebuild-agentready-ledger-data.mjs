...
const decisionGate = (parent_id, id, milestone, horizon, workstream, title, options = {}) => manual(parent_id, id, "DECISION_GATE", milestone, horizon, workstream, title, "JEASON", options.status ?? "DECISION_REQUIRED", { ...options, decision_gate: { metrics: options.metrics ?? [], review_date: null, allowed_outcomes: ["CONTINUE", "CORRECT", "PAUSE", "PIVOT", "REJECT"], selected_outcome: null, owner_approval: null } });
...
for (const row of [["AR-ENG-002","Add benchmark metric calculation"],["AR-ENG-003","Correct AR001 AR003 AR008 and AR010 semantics"],["AR-ENG-004","Add performance and reproducibility benchmark"],["AR-ENG-005","Publish reproducible benchmark report and limitations"],["AR-ENG-006","Prepare voluntary false-positive reporting command"]]) codex("AR-ENG-EPIC", row[0], row[0] === "AR-ENG-006" ? "M5" : "M4", "BEFORE_PRO_TECHNICAL_COMPLETION", "ENG", row[1], { weight: row[0] === "AR-ENG-003" ? 5 : 3, depends_on: row[0] === "AR-ENG-002" ? ["AR-ENG-001H"] : row[0] === "AR-ENG-005" ? ["AR-ENG-001H", "AR-ENG-002", "AR-ENG-003", "AR-ENG-004"] : ["AR-ENG-002"], source_documents: [doc.engine, doc.rules], branch: slug(row[1]), pr_title: `${row[0] === "AR-ENG-003" ? "fix" : row[0] === "AR-ENG-006" ? "feat" : "test"}(engine): ${row[1].toLowerCase()}`, allowed_paths: ["agentready-core/**", "scripts/**", "docs/agentready/**"], forbidden_paths: ["package.json", "LICENSE", "*.html"], required_commands: ["node agentready-core/tests/run-agentready-core-tests.mjs"], scope_justification: row[0] === "AR-ENG-003" ? "Weight 5 justified: strongly coupled rule semantics correction across four known rule gaps." : undefined });
...
codex("AR-SITE-EPIC","AR-LAUNCH-001","M7","BEFORE_GLOBAL_LAUNCH","LAUNCH","Run AgentReady Community and Pro launch audit",{owner:"CODEX_AND_JEASON",weight:5,depends_on:["AR-SITE-025","AR-SEC-004","AR-FIN-001","AR-REL-003"],source_documents:[doc.launch],branch:"qa-launch-agentready-community-pro",pr_title:"qa(launch): run AgentReady Community and Pro launch audit",required_evidence:["launch audit report","owner launch decision"],manual_actions:["Owner reviews final launch audit"],decision_gate:{metrics:["launch QA pass","legal pass","payment pass","support readiness"],review_date:null,allowed_outcomes:["CONTINUE","CORRECT","PAUSE","PIVOT","REJECT"],selected_outcome:null,owner_approval:null},scope_justification:"Weight 5 justified: final audit is a single decision gate and does not implement multiple systems."});
...
decisionGate("AR-SITE-EPIC","AR-MARKET-001","M7","BEFORE_GLOBAL_LAUNCH","MARKET","Evaluate first Community users and Pro price signal",{depends_on:["AR-COM-006"],source_documents:[doc.gtm],required_evidence:["decision gate record"],metrics:["first users","retention 30 days","Pro interest","false positive feedback"]});
