...
// -------- Helpers: arg parsing (supports repeated flags like --label) --------
  ...
  while (i < argv.length) {
    ...
    if (arg.startsWith("--")) {
      ...
      if (next && !next.startsWith("--")) {
        // Accumulate repeated flags (e.g. --label a --label b)
        if (Object.prototype.hasOwnProperty.call(result.flags, name)) {
          const prev = result.flags[name];
          if (Array.isArray(prev)) {
            prev.push(next);
          } else {
            result.flags[name] = [prev, next];
          }
        } else {
...
function cmdHash(args) {
  ...
  if (!file) exitWithError("Usage: timeproofs hash <file>");
  if (!fs.existsSync(file)) exitWithError(`File not found: ${file}`);
...
async function cmdTimestamp(args) {
  ...
  if (!file) {
    exitWithError(
      "Usage: timeproofs timestamp <file> [--api <url>] [--type <type>] [--domain <domain>] [--purpose <purpose>] [--mime <mime>] [--label <label> ...] [--notes <text>] [--out <path>]"
    );
  }
  ...
  if (Array.isArray(labelFlag)) labels.push(...labelFlag);
  else if (typeof labelFlag === "string") labels.push(labelFlag);
  ...
  try {
    timestampResponse = await TimeProofsV02.timestamp(hashHex, { baseUrl: apiBase });
  } catch (e) {
    exitWithError(
      "TimeProofs timestamp error: " + (e && e.message ? e.message : String(e))
    );
  }
  ...
  const issuerMsg =
    ...
    timestampResponse.timestamp.issuer
      ? timestampResponse.timestamp.issuer
      : apiBase;

  log("✔ Timestamp issued by " + issuerMsg);

  // 3) Build meta object (local-only)
  const meta = {};
  let hasMeta = false;

  if (type) { meta.type = type; hasMeta = true; }
  if (domain) { meta.domain = domain; hasMeta = true; }
  if (purpose) { meta.purpose = purpose; hasMeta = true; }
  if (mime) { meta.mime = mime; hasMeta = true; }
  if (labels.length > 0) { meta.labels = labels; hasMeta = true; }
  if (notes) { meta.notes = notes; hasMeta = true; }
  ...
  if (hasMeta) bundleInput.meta = meta;
  ...
  try {
    ...
    exitWithError(
      "Error while creating bundle: " + (e && e.message ? e.message : String(e))
    );
  }
  ...
  // 6) Save bundle
  try {
    fs.writeFileSync(targetOut, JSON.stringify(bundle, null, 2), "utf8");
  } catch (e) {
    exitWithError(
      "Failed to write bundle file: " + (e && e.message ? e.message : String(e))
    );
  }
...
async function cmdVerify(args) {
  ...
  if (!bundlePath) exitWithError("Usage: timeproofs verify <bundle> [--file <file>]");
  if (!fs.existsSync(bundlePath)) exitWithError(`Bundle file not found: ${bundlePath}`);
  ...
  try {
    ...
    exitWithError(
      "Failed to read or parse bundle JSON: " + (e && e.message ? e.message : String(e))
    );
  }

  let fileBytes = null;
  if (filePath) {
    if (!fs.existsSync(filePath)) exitWithError(`File specified with --file not found: ${filePath}`);
    try {
      const buf = fs.readFileSync(filePath);
      fileBytes = new Uint8Array(buf);
    } catch (e) {
      exitWithError(
        "Failed to read file for --file: " + (e && e.message ? e.message : String(e))
      );
    }
  }
  ...
  try {
    result = fileBytes
      ? await TimeProofsV02.verifyBundle(bundleJson, { file: fileBytes })
      : await TimeProofsV02.verifyBundle(bundleJson);
  } catch (e) {
    exitWithError(
      "Error during bundle verification: " + (e && e.message ? e.message : String(e))
    );
  }
  ...
  if (!ok) exitWithError("Bundle verification FAILED", 2);
...
async function main() {
  ...
  if (cmd === "hash") return cmdHash(args);
  if (cmd === "timestamp") return cmdTimestamp(args);
