...
assert.ok(workflow.includes("assert.equal(seoBatch?.status, 'READY')"));
...
assert.match(workflow, /Batch ID: ARB-SITE-GLOBAL-006/);
assert.match(workflow, /Base: site-agentready-global-docs-adoption/);
assert.match(workflow, /Branch: site-agentready-global-discovery/);
assert.match(workflow, /Draft PR target: site-agentready-global-docs-adoption/);
assert.match(workflow, /git merge-base --is-ancestor 6b22fda5e6a5d3a39bddc6dc04a479e228b7199e HEAD/);
assert.match(workflow, /assert\.doesNotMatch\(nextPrompt, \/Exact approved base head: 6b22fda5e6a5d3a39bddc6dc04a479e228b7199e\/\)/);
