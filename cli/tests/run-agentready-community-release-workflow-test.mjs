...
assert.ok(workflow.includes("assert.equal(standardBatch?.status, 'PLANNED')"));
assert.ok(workflow.includes("assert.equal(standardBatch?.spec_status, 'SKELETON')"));
...
assert.match(workflow, /not EXECUTION_READY/);
