...
assert.ok(workflow.includes("assert.equal(next?.batch?.id, 'ARB-SITE-GLOBAL-006')"));
assert.ok(workflow.includes("assert.equal(next?.action_type, 'REVIEW_OR_MERGE')"));
assert.ok(workflow.includes("assert.equal(next?.action_owner, 'JEASON')"));
...
assert.match(workflow, /JEASON - ARB-SITE-GLOBAL-006 - Publish SEO GEO AI-first structured data and international architecture/);
assert.match(workflow, /assert\.doesNotMatch\(nextPrompt, \/Batch ID: ARB-SITE-GLOBAL-007\/\)/);
assert.match(workflow, /assert\.doesNotMatch\(nextPrompt, \/Branch: qa-agentready-global-standard-site\/\)/);
