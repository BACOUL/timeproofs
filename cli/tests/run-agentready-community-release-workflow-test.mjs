...
assert.ok(workflow.includes("const seoBatch = batches.get('ARB-SITE-GLOBAL-006')"));
assert.ok(workflow.includes("const qaBatch = batches.get('ARB-SITE-GLOBAL-007')"));
assert.ok(workflow.includes("const trustOwnerAcceptance = (trustBatch?.evidence || []).find((entry) => entry.type === 'owner_review_acceptance')"));
assert.ok(workflow.includes("const docsOwnerAcceptance = (docsBatch?.evidence || []).find((entry) => entry.type === 'owner_review_acceptance')"));
assert.ok(workflow.includes("const seoOwnerAcceptance = (seoBatch?.evidence || []).find((entry) => entry.type === 'owner_review_acceptance')"));
...
assert.ok(workflow.includes("assert.equal(trustOwnerAcceptance?.previous_reconciled_head, '943d9fea90748a0496ce872dc48b14253eb3a16b')"));
assert.ok(workflow.includes("assert.equal(trustOwnerAcceptance?.current_reconciled_head, '50415ac768194a1448cc1081b1b9a60b3b299b96')"));
assert.ok(workflow.includes("assert.equal(trustBatch?.stacked_child_head_sha, '50415ac768194a1448cc1081b1b9a60b3b299b96')"));
assert.ok(workflow.includes("assert.equal(docsBatch?.status, 'IN_REVIEW')"));
...
assert.ok(workflow.includes("assert.equal(docsBatch?.stacked_base_head_sha, '50415ac768194a1448cc1081b1b9a60b3b299b96')"));
assert.ok(workflow.includes("assert.equal(docsBatch?.pr_number, 137)"));
assert.ok(workflow.includes("assert.equal(docsBatch?.stacked_execution_can_continue, true)"));
assert.ok(workflow.includes("assert.equal(docsBatch?.stacked_child_head_sha, '6b22fda5e6a5d3a39bddc6dc04a479e228b7199e')"));
assert.ok(workflow.includes("assert.equal(docsOwnerAcceptance?.reviewed_public_content_head, '6b22fda5e6a5d3a39bddc6dc04a479e228b7199e')"));
assert.ok(workflow.includes("assert.equal(docsOwnerAcceptance?.owner_review_complete_for_stacked_continuation, true)"));
assert.ok(workflow.includes("assert.equal(seoBatch?.status, 'IN_REVIEW')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.spec_status, 'EXECUTION_READY')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.owner, 'CODEX_AND_JEASON')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.base_branch, 'site-agentready-global-docs-adoption')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.branch, 'site-agentready-global-discovery')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.pr_title, 'site(discovery): publish SEO GEO and international architecture')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.pr_number, 138)"));
assert.ok(workflow.includes("assert.equal(seoBatch?.stacked_on_batch, 'ARB-SITE-GLOBAL-005')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.stacked_base_head_sha, '3cb404b133ee1db01024dc9cba52cd7f8b1faeed')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.stacked_required_ancestor_sha, '6b22fda5e6a5d3a39bddc6dc04a479e228b7199e')"));
assert.ok(workflow.includes("assert.equal(seoBatch?.stacked_execution_can_continue, true)"));
assert.ok(workflow.includes("assert.equal(seoOwnerAcceptance?.reviewed_public_content_head, '2d07223132dc5ec009b8c38a68a7b4aacb0a6163')"));
assert.ok(workflow.includes("assert.equal(seoOwnerAcceptance?.route_count, 34)"));
assert.ok(workflow.includes("assert.equal(seoOwnerAcceptance?.corrected_visual_hierarchy_accepted, true)"));
assert.ok(workflow.includes("assert.equal(seoOwnerAcceptance?.owner_review_complete_for_stacked_continuation, true)"));
assert.ok(workflow.includes("assert.equal(qaBatch?.status, 'READY')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.spec_status, 'EXECUTION_READY')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.owner, 'CODEX_AND_JEASON')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.base_branch, 'site-agentready-global-discovery')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.branch, 'qa-agentready-global-standard-site')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.pr_title, 'qa(site): validate complete global standard site')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.stacked_on_batch, 'ARB-SITE-GLOBAL-006')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.stacked_required_ancestor_sha, '2d07223132dc5ec009b8c38a68a7b4aacb0a6163')"));
assert.ok(workflow.includes("assert.equal(qaBatch?.stacked_base_head_sha, undefined)"));
assert.ok(workflow.includes("assert.equal(next?.batch?.id, 'ARB-SITE-GLOBAL-007')"));
assert.ok(workflow.includes("assert.equal(next?.action_type, 'READY')"));
assert.ok(workflow.includes("assert.equal(next?.action_owner, 'CODEX_AND_JEASON')"));
assert.ok(workflow.includes("assert.equal(counts.execution_batches.immediately_executable, 1)"));
assert.match(workflow, /assert\.match\(nextPrompt, \/Batch ID: ARB-SITE-GLOBAL-007\/\)/);
assert.match(workflow, /assert\.match\(nextPrompt, \/Base: site-agentready-global-discovery\/\)/);
assert.match(workflow, /assert\.match\(nextPrompt, \/Branch: qa-agentready-global-standard-site\/\)/);
assert.match(workflow, /assert\.match\(nextPrompt, \/Draft PR target: site-agentready-global-discovery\/\)/);
assert.match(workflow, /assert\.match\(nextPrompt, \/git merge-base --is-ancestor 2d07223132dc5ec009b8c38a68a7b4aacb0a6163 HEAD\/\)/);
assert.match(workflow, /assert\.doesNotMatch\(nextPrompt, \/Exact approved base head:\/\)/);
