...
TimeProofs is the privacy-first traceability layer for observable actions performed by AI systems, agents, and automations. Each observable AI action creates a clear, signed and verifiable traceability file. The company keeps that file wherever it wants. TimeProofs seals the file fingerprint, not sensitive content.
...
- Can the evidence file be checked for later modification?

Traditional technical logs are useful, but they are often fragmented, hard to share, hard to read outside engineering, and not designed as customer-owned traceability records. TimeProofs does not replace those logs. It creates a separate action traceability file that can reference technical evidence while remaining portable and verifiable.

## Product Truths To Preserve

- TimeProofs is the black box for AI actions.
- Each observable AI action should produce a clear, signed and verifiable traceability file.
- The customer keeps the traceability file in its own storage, systems, evidence repository, document vault, ticket, CRM, data lake, or legal archive.
...
- At a specific timestamp, TimeProofs received a cryptographic fingerprint for a traceability file.
...
This creates evidence of file integrity and existence at a point in time. It helps a company show that an action traceability file existed in a given form when it was sealed.
...
1. The customer creates the Action File in its own environment.
...
Sensitive prompts, outputs, documents, user data, business data, personal data, customer names, internal notes, and decision details should stay outside TimeProofs by default. Customers can decide what their local Action File contains, but the service boundary remains the fingerprint.

## TimeProofs Action File v1

**TimeProofs Action File v1** is the first sellable product.

It is a portable traceability file for one observable AI action. It should be human-readable enough for operations, support, risk, sales, and leadership teams, while structured enough for developers to generate and verify automatically.

A V1 Action File should answer:
...
The Action File can include local customer signatures, internal IDs, references to logs, references to tickets, model or agent identifiers, redaction notes, and operational context. TimeProofs signs the sealing receipt for the fingerprint; it does not need to read the file content.

## V1 Product Boundaries

For V1, the product should stay deliberately small:

- Define the Action File format.
...
- Produce a portable action file for customers, partners, support, risk review, and internal accountability.
...
The first sales validation should focus on whether buyers understand and value the Action File, not whether they want a full dashboard or a broad compliance platform.
